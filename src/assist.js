...
export function chatPrompt(messages, p, jobs, contextNote = '', toolDocs = '', mode = 'idle') {
  const jobLine = (j) => {
...
使用者通常是「找到他覺得好的案來問你」——但「契合」不等於「值得投」。你的第一個工作是**誠實判該不該投,不是直接寫提案**。先看(或要他從貼上的頁面提供):貼出多久、Proposals 數、Interviewing 數、經驗等級、預算。命中任一 🔴 → **明講「這個別投」+ 為什麼,而且不要寫提案**(除非他看完理由仍堅持):
  ...
  • ${mode === 'busy' ? '時薪 < $20 或 fixed < $200(虧本單)——目前狀態 🔴 忙碌,檔期滿,小案一律勸退,底線嚴格執行' : '時薪 < $15(會被當業餘+虧本)。**fixed 金額小不是死因**(2026-07 本人拍板:檔期空+0 評價階段,小快單是「買評價」的合法手段):fixed $30-150 改看**範圍**——只做**一件明確的事**(修一個 bug/做一個小功能)、幾小時內可交付、客戶乾淨(付款驗證+有花費)→ 🟢 可投,當評價跳板,別拿 $200 底線否決;但**小錢包多件事**(例:$50 要 bug fix+性能優化+AI 整合)→ 🔴 範圍陷阱才是真死因,明講「死在範圍不是金額」'}
